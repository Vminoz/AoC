...
def neighbors2d(index, shape):
    ...
    valid = ((neighbors[:, 0] >= 0) & (neighbors[:, 0] < shape[0]) &
...
def pathfinder(mountain:np.ndarray, start, end, animate=False, do_fail_plot=False):
    ...
    bias = np.array([[IGNORE*(1 - ((i,j) == start)) +
                      (abs(end[0]-i) + abs(end[1]-j))
    ...
    while (bias < IGNORE).any():
        ...
        if u == end:
            if animate: m_map.flip_pov()
            path = backtrack(u, start, prev)
            if animate: m_map.show_path(path)
            return True, path
        bias[u] += IGNORE
        visited[u] = True
        nbs, nbs_c = update_neighbors(mountain, mshape, dist, bias, prev, u)

        if animate: m_map.update(visited, nbs, nbs_c)

    if animate: m_map.update(visited,[],[])
    if do_fail_plot: fail_plot(dist, bias)
...
def parse_input(file_name:str) -> np.ndarray:
    with open(file_name) as f:
        mountain = [[ord(lett)-97
                     for lett in line.rstrip()]
                    for line in f]
