...
class CaveScan:
    ...
    def __init__(
        ...
        self.source = (source[0] - self.lt[0], source[1] - self.lt[1])
        ...
        if animate:
            ...
            self.fig = plt.figure(figsize=(fdpi * self.shape[0], fdpi * self.shape[1]))
    ...
    def __str__(self) -> str:
        return "\n".join(
            ["".join(CaveScan.SYMBOLS[val] for val in row) for row in self.model.T]
        )

    def _mkframe(self):
        self.img.set_data(self.model.T)
        self.fig.savefig(
            f"frames/{1 * (not self.floor) + 2 * (self.floor)}/"
