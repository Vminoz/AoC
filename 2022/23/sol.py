...
def move_elf(elf:tuple[int,int], elves:set, order:DQ):
    ...
    if not any(nb_elf): return elf
    for mask in order:
        if all(not nb_elf[i] for i in mask): return nbs[mask[1]]
    return elf
...
def step_elves(elves:set, order:DQ, verbose:bool=False) -> tuple[set,bool]:
    ...
    for elf in elves:
        ...
        if not moving and new_pos != elf:
            moving = True
        if new_pos in moves:
            if verbose: print('Two going to', new_pos, 'none will')
            other_elf = moves.pop(new_pos)
            moves[other_elf] = other_elf
            moves[elf] = elf
        else:
            moves[new_pos] = elf
            if verbose: print(elf, '→', new_pos)
