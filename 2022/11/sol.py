...
class Monkey:
    ...
    def throw_to(self, monkeys:list, verbose=False):
        while self.items:
            self.inspect_cnt += 1
            item = self.items.pop()
            if verbose: print('Monke looks at item with WL:', item)
            item = self.op(item)
            if self.worry_clip:
            ...
            if verbose: print(' → Item is now WL:', item)
            if verbose: print(' → Item thrown to', target)
...
def parse_input(file_name) -> list[Monkey]:
    ...
    with open(file_name) as f:
        for line in f:
            if line[:3] != '  S':
                continue
            monkey = Monkey(deque(int(num)
                for num in reversed(line[18:-1].split(', '))))
            line = next(f)
            operator = line[23]
            operand = line[25:-1]
            if operand == 'old':
