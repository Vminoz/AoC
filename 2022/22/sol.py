        ...
        for i, line in enumerate(f):
            ...
            if not line:
                ...
                map_arr = np.vstack(
                    (
                        map_arr,
                        [SYMBOL[line[i]] if i < l_len else 0 for i in range(r_len)],
                    )
                )
            else:
                instructions = []
...
def slide(
    pos: np.ndarray, direction: int, n: int, map_arr: np.ndarray, warp_dict: dict
):
    ...
    for _ in range(n):
        ...
        if out_of_bounds(new_pos, map_arr):
            if warp_dict:
                ...
                new_pos -= dir_v * valid_width(
                    map_arr, 1 - direction % 2, pos[direction % 2]
                )
...
def out_of_bounds(pos: np.ndarray, map_arr: np.ndarray) -> bool:
    return (pos < 0).any() or (pos >= map_arr.shape).any() or not map_arr[tuple(pos)]


def is_armpit(position: tuple[int, int], map_arr: np.ndarray) -> bool | np.ndarray:
    if out_of_bounds(np.array(position), map_arr):
        return False
    for d in FACING:
    ...
    for d in DIAGON:
        ...
        if out_of_bounds(pos, map_arr):
            if armpit_normal is None:
                ...
                return False  # Can't have two armpit holes
...
def draw_path(map_arr: np.ndarray, instructions: list, warp_dict: dict = None):
    position = np.array(
        (0, np.argmax(map_arr[0, :] == 1))
    )  # Argmax shorthand for "first True"
    direction = 0
    turning = True
