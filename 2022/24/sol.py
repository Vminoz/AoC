...
class Valley:
    ...
    def __str__(self) -> str:
        return "\n".join(
            [
                "".join(
                    Valley.SYMBOLS[int(np.log2(val))] if val else Valley.SYMBOLS[-1]
                    for val in row
                )
            ...
            for exp, direction in enumerate(DIRECTION[:-1]):
                if v & 1 << exp:
                    ...
                    if self.field[new_pos] & 16:
                        ax = exp % 2
                        new_pos = tup_swap(
                            (i, j), ax, self.field.shape[ax] - 1 - (i, j)[ax]
                        )
                        if verbose:
                            print("↑ wall in axis", ax)
                            print(
                                "Replaced by", self.field.shape[ax] - 1, "-", (i, j)[ax]
                            )
    ...
    def from_file(cls, file_name):
        with open(file_name) as f:
            field = np.array(
                [
                    [
                        1 << cls.SYMBOLS.index(ch) if cls.SYMBOLS.index(ch) < 5 else 0
                        for ch in line
                    ]
