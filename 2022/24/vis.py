...
from matplotlib import pyplot as plt, animation as ma, colors as mc

from sol import Valley, valley_a_star

def animate_path(valley:Valley, path:list[tuple[int,int]],
                 visited:list[set[tuple[int,int]]], gif=False):
        ...
        scat = ax.scatter(x=p[1][0], y=p[0][0],
                    c='r', s=40, zorder=100, marker='s')
        visit_scat = ax.scatter(x=p[1][0], y=p[0][0],
                    c='g', s=40, zorder=80, marker='s')
        ...
        anim = ma.FuncAnimation(fig, animation_update,
                                frames=len(path), blit=True)
        if gif:
