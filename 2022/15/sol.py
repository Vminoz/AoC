...
from sensor_class import Sensor, parse_sensors
...
def scan_y(sensors: set[Sensor], y_coord: int, verb=False, bounds=None):
    ...
    for sensor in sensors:
        if verb:
            print("looking at sensor", sensor)
        if interv := sensor.get_x_interval(y_coord):
            if verb:
                print(f"Merge {interv} into\n{intervals}")
    ...
    if bounds is not None:
        ...
        if verb:
            print(f"Bound intervals:\n{intervals}")
    else:
        for b in beacons:
            if verb:
                print("Splitting at", b, "due to beacon.")
            intervals.split(b)
    return intervals
...
def dead_cell(sensors: set[Sensor], lb=0, ub=4_000_000, verb=False) -> tuple[int, int]:
    for y in range(lb, ub + 1):
        ...
        if verb and not y % 100000:
            print(f"{y=} → {scan}", end="\r")
        if scan[0] != [lb, ub]:
            if verb:
                print(f"Found something interesting! {y=} → {scan}")
            if scan[0][0] == lb + 1:
                return lb, y
            if scan[0][1] == ub - 1:
                return ub, y
...
def main():
    INPUTS = {"s": ("in.txt", 10, 20), "h": ("input.txt", 2_000_000, 4_000_000)}
    MODE = "h"
