...
class NotTetris:
        ...
        for y in reversed(range(self.height+1)):
            ...
            for x in range(self.width):
                if (x,y) in self.blocks or(
                          self.active_rock is not None and
                          (self.active_rock == (x,y)
                           ).all(axis=1).any()):
                    row += '█'
                else:
                    row += '░'
        ...
        for cell in new_pos:
            ...
            if tuple(cell) in self.blocks:
                if stopping:
                    ...
                    self.block_height = max(self.block_height,
                                      self.active_rock[-1,1])
                    self.active_rock = None
                return True
        self.active_rock = new_pos
        self.height = max(self.block_height,
                          self.active_rock[-1,1])
        return False

    def simulate(self, jets:list, n_rocks:float,
                 manual:bool=False, probe:int=2022) -> int:
        self.jets = jets
        self.manual = manual
        ji, nj = 0, len(jets)
        ...
        for rock_count in range(int(n_rocks)):
            if rock_count == probe: print(f'{probe} rocks; height:', self.height)
            ...
            if rock_count > probe and key in cache:
                ...
                if not overshoot:
                    ...
                    print(f'Found nice cycle of length {cycle_length} after ' +
    ...
    def _drop_active(self, ji):
        while self.active_rock is not None:
            ...
            if self.manual:
                print(self)
                self.manual = not input(f'Jet: {chr(j+61)}, step? (not blank → skip) ')
            self.step(j)
